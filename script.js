...
function setupEventListeners() {
    ...
    apiKeyForm.addEventListener('submit', async (e) => {
        ...
        const originalText = apiKeyBtn.textContent;
        ...
        try {
            ...
            let errorMessage = 'OpenAI 초기화에 실패했습니다.';
            
            if (error.message.includes('API 키가 유효하지 않습니다')) {
                errorMessage = 'API 키가 유효하지 않습니다. 다시 확인해주세요.';
            } else if (error.message.includes('네트워크 연결')) {
                errorMessage = '네트워크 연결을 확인해주세요.';
            } else if (error.message.includes('quota')) {
                errorMessage = 'API 사용 한도가 초과되었습니다. OpenAI 계정을 확인해주세요.';
            }
            
            alert(errorMessage);
        } finally {
            apiKeyBtn.disabled = false;
            apiKeyBtn.textContent = originalText;
        }
    });
...
async function generatePhrases(name, mood, language) {
    ...
    try {
        ...
        let errorMessage = '문구 생성 중 오류가 발생했습니다.';
        
        if (error.message.includes('quota')) {
            errorMessage = 'API 사용 한도가 초과되었습니다. OpenAI 계정을 확인해주세요.';
        } else if (error.message.includes('rate limit')) {
            errorMessage = 'API 호출 속도 제한에 걸렸습니다. 잠시 후 다시 시도해주세요.';
        } else if (error.message.includes('network') || error.message.includes('fetch')) {
            errorMessage = '네트워크 연결을 확인해주세요.';
        } else if (error.message.includes('Invalid API key')) {
            errorMessage = 'API 키가 유효하지 않습니다. 새로고침 후 다시 시도해주세요.';
        }
